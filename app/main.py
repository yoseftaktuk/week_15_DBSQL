...
def customers_credit_limit_outliers():
    ...
    info = connection.get_info(dal.get_customers_by_credit_limit_range())
    return {'data': info}

@app.get("/q2/orders-null-comments")
def orders_null_comments():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_orders_with_null_comments())
    return {'data': info}

@app.get("/q3/customers-first-5")
def customers_first_5():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_first_5_customers())
    return {'data': info}

@app.get("/q4/payments-total-average")
def payments_total_average():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_payments_total_and_average())
    return {'data': info}

@app.get("/q5/employees-office-phone")
def employees_office_phone():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_employees_with_office_phone())
    return {'data': info}

@app.get("/q6/customers-shipping-dates")
def customers_shipping_dates():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_customers_with_shipping_dates())
    return {'data': info}

@app.get("/q7/customer-quantity-per-order")
def customer_quantity_per_order():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_customer_quantity_per_order())
    return {'data': info}

@app.get("/q8/customers-payments-by-lastname-pattern")
def customers_payments_by_lastname_pattern():
    connection = sql_conetion.Sql_connector()
    info = connection.get_info(dal.get_customers_payments_by_lastname_pattern())
    return {'data': info}
